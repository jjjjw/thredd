...
var Comment = exports.Comment = React.createClass({
  render: function () {
    ...
    var userId =  user ? user.id : '54111be98ca49e6f95b202c2';
    ...
    return React.DOM.div({className: 'comment', id: 'comment-' + comment.id}
      , React.DOM.div({className: 'comment-header'}
        , React.DOM.a({className: 'user-name', href: '/users/' + userId }, userName)
        , React.DOM.a({className: 'comment-anchor-link', href: '#comment-' + comment.id}
      ...
      , React.DOM.div({className: 'comment-footer'}
        , React.DOM.div({className: 'comment-actions'}, this.props.actionsDisabled ? '' : actions)
        )
      , Thread({
...
var Thread = exports.Thread = React.createClass({
  render: function () {
    ...
    return React.DOM.div({ className: 'thread thread-depth-' + this.props.depth }, comments);
  }
});
