...
module.exports = React.createClass({
  ...
  , onErr: function (err) {
    this.setState({error: 'There was an error', warn: ''});
    setTimeout(function () {
      this.setState({error: ''});
    }.bind(this), 5000);
  }
  , onWarn: function (message) {
    this.setState({error: '', warn: message});
    setTimeout(function () {
      this.setState({warn: ''});
    }.bind(this), 5000);
  }
  , handleSubmit: function (e) {
    e && e.preventDefault();
    if (!this.state.text) {
      this.onWarn('Please enter a comment')
      return;
    }
    ...
    return React.DOM.div({className: 'editor'}
      , React.DOM.form({onSubmit: this.handleSubmit}
        ...
        , React.DOM.div({className: 'editor-actions'}
          ...
          , React.DOM.span({className: 'post-as'}, ' as ')
          , User()
          , React.DOM.span({className: 'warn-msg'}, this.state.warn)
